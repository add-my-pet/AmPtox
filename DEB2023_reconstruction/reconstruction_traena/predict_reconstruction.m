...
%  s_M = 4.4; % calculated beforehand to speed up the calculation
 tT = temp.tL; %[temp.tL(:,1) C2K(temp.tL(:,2))] ; % K, temperature 
...
EH0 = 0; % this is wrong but since we only look at growth then we don't need to follow maturity explicitly and we save time by not needed to compute the initial maturity based on initial values of L, E and a -
LEH0  = [L0; E0; EH0;0]; % 4-1 vector [J, cm, J, J] with initial conditions

time = data.tL(:,1);

[~, LEH] =  ode23s(@dget_LEH, time, LEH0,[],tyf, tT, par, cPar, s_M); 
